...
dict_p word_counting(int argc, char* argv[])
{
    ...
    //Generated by the master node and sent to every process
    files_info data[num_proc];
    //init data structs
    for(int i = 0; i < num_proc; i++)
        data[i] = files_info_init(argc - 1);
    
    files_info proc_data = files_info_init(argc - 1);
    ...
    if(rank == 0){
        ...
        for(int i = 0; i < num_proc; i++){
            ...
            data[i].num_files = index + 1;

            //each proc should check is his last char is a word terminator, otherwise must do some right padding
            //the right padding is made by removing, for each char needed, one byte until 
            //a terminator symbol is found (terminator symbols are \t, \n, ' ', ...).
            //the last process doesn't do these operations
            if(i + 1 < num_proc){
                int file_size = file_sizes[index];
                int file_index = data[i].files[index].index;
        ...
        files_info_print(rank, proc_data);

        word_counter(proc_data, proc_dict, argv);
        //dict_print(rank, proc_dict);
    }
    //Slaves
    else {
        int data_infos[4];
